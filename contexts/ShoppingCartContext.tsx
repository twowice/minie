...
export function CartProvider({ children }: CartProviderProps) {
   ...
   const normalizeItem = (item: any): CartItem => ({
      ...
      title: item.name || '',
      brand: item.brand || '',
      image: item.image || 'https://cdn.imweb.me/thumbnail/20220924/2f7f6930fd5c3.png',
      price: item.price || 0,
      num: item.num || 1,
   ...
   const { totalCostPrice, totalDiscountAmount, totalPrice } = useMemo(() => {
      ...
      const cost = checkedItems.reduce((acc, item) => acc + item.price * (item.num ?? 1), 0);
      const discount = checkedItems.reduce((acc, item) => acc + (item.discountAmount ?? 0) * (item.num ?? 1), 0);
      return {
         totalCostPrice: cost,
   ...
   const buyNow = useCallback(
      async (newItem: any) => {
         ...
         if (exists) {
            ...
            updateCartItems([{ product_id: targetId, product_num: newQty }]).catch(err =>
      ...
      async (item: CartItem) => {
         if (isLiked(item.id)) {
            await deleteLikedItem(item.id).then(
               res => res && setLikedItems(prev => prev.filter(i => i.id !== item.id)),
            );
         } else {
   ...
   const toggleLike = (item: any) => {
      isTypedCartItem(item) ? toggleLikeFromCart(item) : toggleLikeFromProduct(item);
   };
   ...
   const toggleCart = useCallback(
      async (item: CartItem) => {
         if (isItemCart(item.id)) {
            await deleteCartItem(item.id).then(res => res && setCartItems(prev => prev.filter(i => i.id !== item.id)));
         } else {
            await addCartItems([{ product_id: item.id, product_num: 1 }]).then(
               res => res && setCartItems(prev => [{ ...normalizeItem(item), checked: false, num: 1 }, ...prev]),
            );
         }
      },
      [isItemCart],
