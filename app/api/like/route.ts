...
export async function GET(request: NextRequest) {
   ...
   const transformedCartItems: CartItem[] = typedRawCartItems
      .map((item: RawCartItem) => {
         if (!item.products) {
            return null;
         } //추가 : 상품 정보(item.products)가 없으면 null을 반환해서 건너뛰게 함
         return {
            id: item.product_id,
            checked: true,
            isUpdated: false,
...
export async function DELETE(request: NextRequest) {
   ...
   if (error) {
      ...
      return NextResponse.json(
         { error: '[server] api/like DELETE Failed to delete liked item: ' + error.message },
         { status: 500 },
      );
   }
...
export async function POST(request: NextRequest) {
   ...
   const { error } = await supabase.from('product_likes').insert({ user_id: Number(uid), product_id: productId });
