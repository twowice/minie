...
export async function GET(request: NextRequest) {
   ...
   if (!paymentKey || !amount || !secretKey) {
      console.error('필수 결제 파라미터 또는 시크릿 키 누락', {
         paymentKey,
         orderId,
         amount,
      ...
      return NextResponse.redirect(
         new URL(`/payment/fail?message=필수 결제 파라미터 누락&orderId=${orderId}`, request.nextUrl.origin),
      );
   }
   ...
   try {
      ...
      if (!response.ok) {
         ...
         return NextResponse.redirect(
            new URL(`/payment/fail?message=${errorMessage}&orderId=${orderId}`, request.nextUrl.origin),
         );
      }
      ...
      if (!isUpdateSuccess) {
         ...
         return NextResponse.redirect(
            new URL(
               `/payment/fail?message=Internal server error during updating order status&orderId=${orderId}`,
