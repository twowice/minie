...
export default function Page() {
   ...
   function content() {
      ...
      if (error) return <Box color="black">오류 발생: {error || '알 수 없는 오류'}</Box>;
      if (orders.length === 0) return <Box color="black">주문 내역이 없습니다.</Box>;

      return currentOrders.map((order, idx) => <TrackingOrderItem key={idx} order={order} />);
   }
   ...
   return (
      <Box color={'black'}>
         <Text color="#000000" fontWeight="bold" fontSize="28px" mb={'16px'}>
            주문 조회
         </Text>
         <HStack
         ...
         <VStack align="start" color="#000000" fontSize="12px" py={'16px'}>
            <Text>
               • 2017년 4월 1일 이후 내역만 조회가 가능하며, 이전의 주문내역은 Minié 주문내역에서 확인하실 수 있습니다.
            </Text>
            <Text>
               • 매장 구매는 포인트 적립을 한 경우, 최근 1년 내역만 조회가 가능합니다. (2019년 9월 27일 이후 내역만 조회
               가능)
            </Text>
         </VStack>
