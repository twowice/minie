...
export default function ShoppingCartDrawer({ headerHeight }: { headerHeight: number }) {
   ...
   const cartItemsIdsAllSet = useMemo(() => new Set(cartItems.map(item => item.id)), [cartItems]);
   const isAllCartChecked = cartItems.length > 0 && cartItems.every(item => item.checked);
   const isAllLikedItemChecked = likedItems.length > 0 && likedItems.every(item => item.checked);
                     ...
                     >
                        <Tabs.List borderBottom={'none'} display={'flex'} w="100%">
                           <Tabs.Trigger
                              ...
                              _focus={{ boxShadow: 'none', outline: 'none' }} // 클릭 후 남는 포커스 링 제거
                              _active={{ background: 'transparent' }} // 클릭 순간 배경색 변함 방지
                              bottom={'-1px'}
                              _selected={{ color: '#FA6D6D', bottom: '0px' }}
                           >
                              <Drawer.Title w={'100%'} fontSize="16px" fontWeight="medium">
                                 장바구니
                              </Drawer.Title>
                           </Tabs.Trigger>
                           <Tabs.Trigger
                     ...
                     >
                        ...
                        >
                           <Checkbox.Root
                              ...
                              variant={'outline'}
                              colorPalette={'red'}
                              checked={currentTabsValue === 'cart' ? isAllCartChecked : isAllLikedItemChecked}
                              onCheckedChange={() => toggleAllChecked(currentTabsValue as 'cart' | 'like')}
                           >
                              <Checkbox.HiddenInput />
                              <Checkbox.Control />
                           </Checkbox.Root>
                           <Box color="#CCCCCC">|</Box>
                           <Box display={'flex'} flex={1} justifyContent={'center'} color={'black'} fontWeight={'bold'}>
                              상품정보
                           </Box>
                        </Box>
                  ...
                  <Drawer.Footer px="40px" flexShrink={0} display={'flex'}>
                     ...
                     {currentTabsValue === 'cart' && (
                        <Button
                           ...
                           bg={'#FA6D6D'}
                           fontWeight={'medium'}
                           onClick={() => (totalPrice > 0 ? moveOrder() : zeroTotalPriceToast())}
                        >
