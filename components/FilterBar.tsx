  ...
  useEffect(() => {
    ...
    if (male && !female) {
      ...
      selected = selected.filter(
        (item) => item.gender === "남성" || item.gender === "여성"
      );
    }
  ...
  const filteredData = () => {
    ...
    if (filterPrice.length > 0) {
      checked = checked.filter((item) =>
        filterPrice.some((price) => item.price <= price)
      );
    }
  ...
  return (
    ...
    >
      <Box
        ...
        backgroundColor={"rgba(204,204,204,0.16)"}
        p={"8px"}
      >
        ...
        >
          ...
          <NativeSelect.Root size={"xs"} w={"60px"} variant={"plain"}>
            ...
            >
              ...
              <option
                value="가격순"
                onChange={() => setFilterPrice(price).sort((a, b) => a - b)}
              >
                가격 순
              </option>
            </NativeSelect.Field>
